...
class TransformerClassifier(IntentClassifier):
    ...
    def __init__(self, component_config=None):
        self.model_name = component_config.get("model_name", "vinai/phobert-base")
    ...
    def train(self, train_data, cfg, **kwargs):
        ...
        training_args = TrainingArguments(
            ...
            per_device_train_batch_size=self.component_config.get("batch_size", 16),
    ...
    def _process_intent_ranking(self, outputs):
        ...
        # confidences = outputs["logits"][0].softmax(-1)
        confidences = np.exp(confidences)/sum(np.exp(confidences))
        # print(confidences)
        intent_names = list(self.label2id.keys())
        intent_ranking_all = zip(intent_names, confidences)
        intent_ranking_all_sorted = sorted(
            intent_ranking_all, key=lambda x: x[1], reverse=True
        )
        print(intent_ranking_all_sorted)
        # intent = intent_ranking_all_sorted[0]
        intent_ranking = [
            {"name": x[0], "confidence": x[1]} for x in intent_ranking_all_sorted[:10]
        ]
    ...
    def _predict(self, text):
        ...
        confidences = outputs["logits"][0]
        softmax_probs = F.softmax(confidences, dim=0)

        # Tìm giá trị xác suất lớn nhất
        max_prob, max_index = torch.max(softmax_probs, dim=0)
